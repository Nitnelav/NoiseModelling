...
class Road_Emission_from_Traffic implements ScriptI {

    String title = 'Compute road emission noise map from road table.'
    String description = 'Compute Road Emission Noise Map from Day Evening Night traffic flow rate and speed estimates (specific format, see input details). ' +
    ...
    String exec(Connection connection, input) {
        ...
        sql.withBatch(100, qry) { ps ->
            ...
            while (rs.next()) {
                k++
                currentVal = GeneralTools.ProgressBar(Math.round(10 * k / nbRoads).toInteger(), currentVal)
                //logger.info(rs)
