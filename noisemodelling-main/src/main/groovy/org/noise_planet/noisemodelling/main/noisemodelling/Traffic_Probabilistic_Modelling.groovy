...
package org.noise_planet.noisemodelling.main.noisemodelling
...
import org.noise_planet.noisemodelling.main.utilities.*
...
class Traffic_Probabilistic_Modelling implements ScriptI {

    String title = 'Road traffic probabilistic modeling'
    String description = 'Compute road traffic probabilistic modeling as describe in <b>Aumond, P., Jacquesson, L., & Can, A. (2018). Probabilistic modeling framework for multisource sound mapping. Applied Acoustics, 139, 34-43. </b>.' +
    ...
    String exec(Connection connection, input) {
        ...
        for (int it = 1; it < nIterations; it++) {
            ...
            for (int i = 0; i < allLevels.size(); i++) {
                ...
                k++
                currentVal = GeneralTools.ProgressBar(Math.round(10*i/(allLevels.size()*nIterations)).toInteger(),currentVal)
                ...
                if (sourceLev.get(idSource)[0] > 0) {
                    ...
                    if (!Double.isNaN(soundLevel[0]) && !Double.isNaN(soundLevel[1])
                        ...
                        if (soundLevels.containsKey(idReceiver)) {
                            ...
                            double[] sumArray = GeneralTools.sumArraySR(soundLevel, sourceLev.get(idSource))
    ...
    class ProbabilisticProcessData {
        ...
        double[] getCarsLevel(int idSource) throws SQLException {
            ...
            int kk = 0
            for (f in list) {
                res_d[kk] = 10 * Math.log10((1.0 / 2.0) * (Math.pow(10, (10 * Math.log10(Math.pow(10, res_LV[kk] / 10))) / 10) + Math.pow(10, (10 * Math.log10(Math.pow(10, res_HV[kk] / 10))) / 10)))
                kk++
            }
